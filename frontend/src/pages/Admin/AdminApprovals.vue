<template>
  <q-page class="admin-page">
    <div class="page-container">
      ...
      <div class="approvals-header-colored q-mb-lg row items-center justify-between q-pa-lg">
        ...
        <div class="col-12 col-md-4 text-right q-mt-md q-md-mt-none flex justify-end relative-position" style="z-index: 2;">
          <div class="stats-widget-glass q-pa-md">
            <div class="text-caption text-white opacity-70 text-weight-bold text-uppercase q-mb-xs">Pending Review</div>
            <div class="row items-baseline justify-end">
              <span class="text-h3 text-weight-bolder text-white">{{ pendingCount }}</span>
              <span class="text-body2 text-white opacity-80 text-weight-bold q-ml-sm">applications</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <q-card flat class="glass-card table-glass-container">
        ...
        <div class="panel-header q-pt-sm">
          <q-tabs
            ...
            @update:model-value="fetchPending"
          >
            <q-tab name="pending" label="Pending Applications" class="text-weight-bold" />
            <q-tab name="rejected" label="Rejected Applications" class="text-weight-bold" />
          </q-tabs>
        </div>
        ...
        >
          ...
          <template #body-cell-actions="props">
            <q-td :props="props" align="right">
              <div class="flex items-center gap-sm justify-end">
                ...
                <template v-else>
                  <q-chip dense class="rejected-chip-3d text-weight-bold q-px-md" icon="block">
                    Rejected
                  </q-chip>
                </template>
              </div>
          ...
          <template #no-data>
            <div class="full-width column flex-center q-py-xl empty-state-glass">
              ...
              <div class="text-h6 text-weight-bold text-grey-8">No applications found</div>
              <div class="text-body2 text-grey-6">There are currently no items matching your criteria.</div>
            </div>
          </template>
        </q-table>
      </q-card>
    ...
    <q-dialog v-model="showVendorInfoModal" transition-show="scale" transition-hide="scale">
      <q-card class="review-dialog-glass vendor-info-dialog">
        <q-card-section class="row items-center justify-between q-pa-md panel-header">
          ...
          <q-btn icon="close" flat round dense color="grey-7" @click="showVendorInfoModal = false" />
        </q-card-section>

        <q-card-section class="q-pa-lg scroll" style="max-height: 65vh;" v-if="selectedVendor">
          <div class="text-center q-mb-lg">
            <div class="info-store-name q-mb-xs">{{ selectedVendor.store?.store_name || 'N/A' }}</div>
            <div class="info-owner-name text-grey-7 q-mb-md">Owned by: {{ selectedVendor.store?.owner?.full_name || selectedVendor.owner_name }}</div>
          ...
          <div class="row q-col-gutter-y-md q-col-gutter-x-xl q-mb-lg">
            <div class="col-12 col-sm-6">
              ...
              <div class="text-subtitle2 text-weight-bold text-dark">{{ selectedVendor.email }}</div>
            </div>
            <div class="col-12 col-sm-6">
          ...
          <div class="map-container-3d" v-if="isValidLocation(selectedVendor)">
            <iframe 
              :src="getMapUrl(selectedVendor.store.latitude, selectedVendor.store.longitude)"
              width="100%" 
              height="200" 
        ...
        <q-card-actions align="right" class="q-pa-md dialog-actions-glass" v-if="selectedVendor">
          <template v-if="selectedVendor.status === 'pending'">
            <q-btn flat label="Reject Application" color="red-8" no-caps class="btn-reject-3d q-px-md" @click="openRejectModal(selectedVendor); showVendorInfoModal = false;" />
            <q-btn unelevated label="Approve Vendor" icon="check_circle" color="white" no-caps class="btn-approve-3d q-px-md q-ml-sm" @click="handleApprove(selectedVendor); showVendorInfoModal = false;" />
          </template>
          <template v-else-if="selectedVendor.status === 'rejected'">
            <q-btn flat label="Close" color="grey-8" no-caps class="btn-3d-outline q-px-md" @click="showVendorInfoModal = false" />
            <q-btn unelevated label="Re-evaluate & Approve" icon="restore" color="white" no-caps class="btn-approve-3d q-px-md q-ml-sm" style="background: linear-gradient(180deg, #f97316 0%, #ea580c 100%) !important;" @click="handleApprove(selectedVendor); showVendorInfoModal = false;" />
          </template>
          <template v-else>
            <q-btn outline label="Close" color="grey-8" no-caps class="btn-3d-outline q-px-md" @click="showVendorInfoModal = false" />
          </template>
        </q-card-actions>
      </q-card>
    </q-dialog>
...
const columns = [
  ...
  { name: 'actions', label: 'Actions', field: 'actions', align: 'right', classes: 'print-hide', headerClasses: 'print-hide' },
]
...
const openVendorInfo = (evt, row) => {
  if (evt && (evt.target.closest('.q-btn') || evt.target.closest('.gap-sm'))) return;
...
.admin-page {
  ...
  background-image: 
    radial-gradient(circle at 0% 0%, rgba(220, 38, 38, 0.05) 0%, transparent 500px),
    radial-gradient(circle at 100% 100%, rgba(15, 23, 42, 0.03) 0%, transparent 400px);
...
.panel-header {
  background: linear-gradient(90deg, rgba(254, 242, 242, 0.7) 0%, transparent 100%);
...
.btn-approve-3d {
  ...
  border-radius: 8px !important;
  font-weight: 700;
  box-shadow: 0 4px 10px rgba(22, 163, 74, 0.3), inset 0 1px 1px rgba(255,255,255,0.4);
  transition: all 0.2s ease;
  border: 1px solid #15803d;
}
.btn-approve-3d:hover { transform: translateY(-2px); box-shadow: 0 6px 12px rgba(22, 163, 74, 0.4); }
...
.btn-reject-3d:hover { background: #fef2f2 !important; transform: translateY(-1px); box-shadow: 0 4px 8px rgba(220, 38, 38, 0.15); }
...
.btn-3d-outline:hover { background: rgba(255, 255, 255, 0.8) !important; transform: translateY(-1px); }

.rejected-chip-3d {
  background: linear-gradient(135deg, #fef2f2, #fee2e2) !important;
  color: #dc2626;
  border: 1px solid rgba(220, 38, 38, 0.2);
  box-shadow: 2px 2px 5px rgba(0,0,0,0.05), inset 1px 1px 0 rgba(255,255,255,0.8);
}
...
@media (max-width: 768px) {
  ...
  .approvals-header-colored > .row { flex-direction: column; align-items: flex-start; }
  .stats-widget-glass { width: 100%; justify-content: flex-start; }
}
