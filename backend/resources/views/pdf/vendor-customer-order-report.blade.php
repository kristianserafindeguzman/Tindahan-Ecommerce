        ...
        <table class="data-table">
            ...
            <tbody>
                @foreach($order->items as $item)
                    <tr>
                        <td>
                            <div style="font-weight: bold;">{{ $item->inventory->product_name ?? 'Unknown Product' }}</div>
                        </td>
                        <td style="text-align: center; font-weight: bold;">{{ $item->quantity }}</td>
                        <td style="text-align: right;">{{ number_format($item->unit_price ?? ($item->subtotal / max($item->quantity, 1)), 2) }}</td>
