...
<head>
    <meta charset="utf-8">
    <title>Vendor Application Approvals Report</title>
    <style>
...
<div class="header">
    <div class="header-left">
        ...
        <h3 class="report-title">Vendor Application Approvals Report</h3>
    </div>
    ...
    <tbody>
        @forelse($approvals as $approval)
            @php
                ...
                $badgeClass = $status === 'pending' ? 'badge-pending' : 'badge-rejected';
            ...
            <tr>
                <td>#{{ $approval->id }}</td>
                <td>
                    <strong>{{ $owner->full_name ?? 'N/A' }}</strong><br>
                    <span style="color: #64748b; font-size: 9px;">{{ $owner->email ?? 'N/A' }}</span><br>
                    <span style="color: #64748b; font-size: 9px;">{{ $owner->phone_number ?? 'N/A' }}</span>
                </td>
                <td><strong>{{ $store->store_name ?? 'N/A' }}</strong></td>
                <td style="max-width: 150px; line-height: 1.3;">{{ $store->address ?? 'N/A' }}</td>
                <td>{!! $statusBadge !!}</td>
                <td>{{ $approval->created_at ? \Carbon\Carbon::parse($approval->created_at)->format('Y-m-d h:i A') : 'N/A' }}</td>
            </tr>
        @empty
            <tr>
