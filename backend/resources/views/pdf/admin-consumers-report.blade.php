...
<head>
    <meta charset="utf-8">
    <title>Registered Consumers Report</title>
    <style>
...
<div class="header">
    <div class="header-left">
        ...
        <h3 class="report-title">Registered Consumers Report</h3>
    </div>
...
<table class="data-table">
    ...
    <tbody>
        @forelse($consumers as $consumer)
            @php
                $isDeleted = $consumer->trashed();
                if ($isDeleted) {
                    $status = 'deleted';
                    $badgeClass = 'badge-deleted';
                } else {
                    $status = strtolower($consumer->user_status ?? 'active');
            ...
            <tr>
                <td><strong>{{ $consumer->full_name ?? 'N/A' }}</strong></td>
                <td>{{ $consumer->email ?? 'N/A' }}</td>
                <td>{{ $consumer->phone_number ?? 'N/A' }}</td>
                <td style="max-width: 150px; line-height: 1.3;">{{ $consumer->address ?? 'N/A' }}</td>
                <td>{!! $statusBadge !!}</td>
                <td>{{ $consumer->created_at ? \Carbon\Carbon::parse($consumer->created_at)->format('Y-m-d') : 'N/A' }}</td>
            </tr>
        @empty
            <tr>
